...
2. XYZ (位置随时间变化图)
(1)内容：逐帧显示轨迹的 x、y 和 z 坐标随时间的变化。
横轴为时间（帧索引或绝对时间），纵轴为对应的坐标值。
(2)图形特征：三条曲线分别代表 x、y 和 z 坐标随时间的变化。
曲线变化趋势可以反映机器人的运动模式，例如前进、转弯、爬升等。
(3)用途：分析机器人轨迹的平移运动（前后、左右、上下）。
检查轨迹中是否有异常运动（如突然跳跃或噪声点）。
